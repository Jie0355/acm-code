...
class Deque {
    private:
        ...
        int l_size, r_size; //左部分数组的长度与右部分数组的长度
    public:
        typedef Deque_Iterator<T, T*, T&> iter;
        //初始化
        Deque() {
            l_size = r_size = 10;
            left = new T[l_size];
        ...
        iter begin() {
            if (head > 0) return iter(&left[head - 1]);
            else return iter(&right[-head]);
        }
        iter end() {
            if (tail > 0) return iter(&right[tail - 1]);
            else return iter(&left[-tail]);
        }
        void clear() {
            head = tail = 0;
        }
